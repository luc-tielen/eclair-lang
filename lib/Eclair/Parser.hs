...
insertSpan nodeId span (SpanMap m) =
  SpanMap (M.insert (unNodeId nodeId) span m)
...
parseText :: FilePath -> Text -> Either ParseError (AST, SpanMap)
parseText path text =
  snd <<$>> f (runState (P.runParserT astParser path text) (0, SpanMap mempty))
  where
